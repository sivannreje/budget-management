...
class EditIncome(budget_management.base_handler.BaseHandler):
    async def post(self):
        ...
        print(f"updating income id={id_} with {update_item}")
        updated = await self._data_access_layer.update_one('incomes', {"_id": bson.ObjectId(id_)}, **update_item)
